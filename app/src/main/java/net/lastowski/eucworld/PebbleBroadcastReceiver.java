...
import static net.lastowski.eucworld.utils.Constants.ACTION_PEBBLE_APP_READY;
import static net.lastowski.eucworld.utils.Constants.ACTION_PEBBLE_APP_SCREEN;
import static net.lastowski.eucworld.utils.Constants.ACTION_REQUEST_HORN;
import static net.lastowski.eucworld.utils.Constants.INTENT_EXTRA_LAUNCHED_FROM_PEBBLE;
import static net.lastowski.eucworld.utils.Constants.INTENT_EXTRA_PEBBLE_APP_VERSION;
import static net.lastowski.eucworld.utils.Constants.INTENT_EXTRA_PEBBLE_DISPLAYED_SCREEN;
import static net.lastowski.eucworld.utils.Constants.PEBBLE_APP_UUID;
import static net.lastowski.eucworld.utils.Constants.PEBBLE_KEY_DISPLAYED_SCREEN;
import static net.lastowski.eucworld.utils.Constants.PEBBLE_KEY_LAUNCH_APP;
import static net.lastowski.eucworld.utils.Constants.PEBBLE_KEY_PLAY_HORN;
import static net.lastowski.eucworld.utils.Constants.PEBBLE_KEY_READY;
...
public class PebbleBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (intent.getAction().equals(Constants.INTENT_APP_RECEIVE)) {
            ...
            if (data.contains(PEBBLE_KEY_READY)) {
                int watch_app_version = data.getInteger(PEBBLE_KEY_READY).intValue();
                /*
                if (watch_app_version < net.lastowski.eucworld.utils.Constants.PEBBLE_APP_VERSION)
            ...
            if (data.contains(PEBBLE_KEY_DISPLAYED_SCREEN)) {
            ...
            if (data.contains(PEBBLE_KEY_PLAY_HORN)) {
                Bundle options = new Bundle();
                int action = SettingsUtil.getWatchActionButton(context);
                if (action == Commons.Action.HORN) {
                    int mode = SettingsUtil.getWatchHornMode(context);
                    options.putInt("mode", mode);
                    if (mode == Commons.Action.HornMode.BLUETOOTH_AUDIO) {
                        if (SettingsUtil.getWatchUseCustomHornSound(context))
                            options.putString("path", SettingsUtil.getWatchCustomHornSoundPath(context));
                    }
                }
