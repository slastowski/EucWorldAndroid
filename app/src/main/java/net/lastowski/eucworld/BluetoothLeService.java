...
import net.lastowski.eucworld.utils.*;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch (intent.getAction()) {
                ...
                case Constants.ACTION_REQUEST_KINGSONG_SERIAL_DATA:
                    ...
                    break;
                case Constants.ACTION_REQUEST_HORN:
                    if (WheelData.getInstance().getWheelType() == WHEEL_TYPE.KINGSONG) {
                        byte[] data = new byte[20];
                        data[0] = (byte) -86;
                        data[1] = (byte) 85;
                    ...
                    if (WheelData.getInstance().getWheelType() == WHEEL_TYPE.GOTWAY) {
                        writeBluetoothGattCharacteristic("b".getBytes());
                    }
                    break;
                case Constants.ACTION_REQUEST_LIGHT_TOGGLE:
                    ...
                    if (WheelData.getInstance().getWheelType() == WHEEL_TYPE.GOTWAY) {
                        switch (WheelData.getInstance().getLightStatus()) {
                            case 0:
                                writeBluetoothGattCharacteristic("T".getBytes());
                                writeBluetoothGattCharacteristic("b".getBytes());
                                WheelData.getInstance().setLightStatus(1);
                                break;
                            case 1:
                                writeBluetoothGattCharacteristic("Q".getBytes());
                                writeBluetoothGattCharacteristic("b".getBytes());
                                WheelData.getInstance().setLightStatus(2);
                                break;
                            default:
                                writeBluetoothGattCharacteristic("E".getBytes());
                                writeBluetoothGattCharacteristic("b".getBytes());
                                WheelData.getInstance().setLightStatus(0);
                                break;
                        }
                    }
                    else
                    if (WheelData.getInstance().getWheelType() == WHEEL_TYPE.INMOTION) {
                        ...
                        InMotionAdapter.getInstance().setLightState(!light);
                    }
                    break;
